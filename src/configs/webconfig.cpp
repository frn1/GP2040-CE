...
const static vector<string> spaPaths     = {"/display-config", "/led-config",     "/pin-mapping",
                                            "/settings",       "/reset-settings", "/add-ons"};
...
std::string getLedOptions() {
  ...
  if (ledOptions.indexUp == -1)
    ledButtonMap["Up"] = nullptr;
  else
    ledButtonMap["Up"] = ledOptions.indexUp;
  if (ledOptions.indexDown == -1)
    ledButtonMap["Down"] = nullptr;
  else
    ledButtonMap["Down"] = ledOptions.indexDown;
  if (ledOptions.indexLeft == -1)
    ledButtonMap["Left"] = nullptr;
  else
    ledButtonMap["Left"] = ledOptions.indexLeft;
  if (ledOptions.indexRight == -1)
    ledButtonMap["Right"] = nullptr;
  else
    ledButtonMap["Right"] = ledOptions.indexRight;
  if (ledOptions.indexB1 == -1)
    ledButtonMap["B1"] = nullptr;
  else
    ledButtonMap["B1"] = ledOptions.indexB1;
  if (ledOptions.indexB2 == -1)
    ledButtonMap["B2"] = nullptr;
  else
    ledButtonMap["B2"] = ledOptions.indexB2;
  if (ledOptions.indexB3 == -1)
    ledButtonMap["B3"] = nullptr;
  else
    ledButtonMap["B3"] = ledOptions.indexB3;
  if (ledOptions.indexB4 == -1)
    ledButtonMap["B4"] = nullptr;
  else
    ledButtonMap["B4"] = ledOptions.indexB4;
  if (ledOptions.indexL1 == -1)
    ledButtonMap["L1"] = nullptr;
  else
    ledButtonMap["L1"] = ledOptions.indexL1;
  if (ledOptions.indexR1 == -1)
    ledButtonMap["R1"] = nullptr;
  else
    ledButtonMap["R1"] = ledOptions.indexR1;
  if (ledOptions.indexL2 == -1)
    ledButtonMap["L2"] = nullptr;
  else
    ledButtonMap["L2"] = ledOptions.indexL2;
  if (ledOptions.indexR2 == -1)
    ledButtonMap["R2"] = nullptr;
  else
    ledButtonMap["R2"] = ledOptions.indexR2;
  if (ledOptions.indexS1 == -1)
    ledButtonMap["S1"] = nullptr;
  else
    ledButtonMap["S1"] = ledOptions.indexS1;
  if (ledOptions.indexS2 == -1)
    ledButtonMap["S2"] = nullptr;
  else
    ledButtonMap["S2"] = ledOptions.indexS2;
  if (ledOptions.indexL3 == -1)
    ledButtonMap["L3"] = nullptr;
  else
    ledButtonMap["L3"] = ledOptions.indexL3;
  if (ledOptions.indexR3 == -1)
    ledButtonMap["R3"] = nullptr;
  else
    ledButtonMap["R3"] = ledOptions.indexR3;
  if (ledOptions.indexA1 == -1)
    ledButtonMap["A1"] = nullptr;
  else
    ledButtonMap["A1"] = ledOptions.indexA1;
  if (ledOptions.indexA2 == -1)
